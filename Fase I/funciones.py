    ...
    while ticker.upper() != 'FIN':
        ...
        if activos != False:
            if len(activos) > 1:
                ...
                opcion = input("Escoja el número del ticker a eliminar por su metodología: ")
...
def alta_activo(matriz, nombre):
    ...
    while nombre.upper() != 'FIN':
        ...
        vol_act = input('Ingrese el volumen de actividad de las últimas 24hs: ')
        ...
        met_op = input(f'Ingrese la metodología de operación asignada {metodos_validos}: ')
        
        while not validar_metodologia(met_op):
            print('1: Scalping', '2: Day Trading', '3: Swing Trading', '4: HODL')
            met_op = input(f'Ingrese la metodología de operación asignada {metodos_validos}: ')
                        
        unidades = input('Ingrese las unidades totales en tesorería: ')
        ...
        if validar_repetidos(nombre, ticker, met_op, matriz):
            ...
            activo = [nombre,ticker.upper(),float(valor_ref),float(vol_act),metodos_validos[int(met_op) - 1],float(unidades),int(punt_conf)]
    ...
    while nombre.upper() != 'FIN':
        ...
        if activo_encontrado:
            ...
            if len(activo_encontrado) > 1:
                ...
                opcion = input("Escoja el nombre a modificar por su metodología: ")
            ...
            while int(opcion) != 8:
                ...
                if int(opcion) == 1:
                    ...
                    activo_encontrado[3] = float(nuevo_vol_act)
                    print("Volumen de actividad del activo modificado exitosamente.")

                elif int(opcion) == 5:
                    metodos_validos = ['Scalping', 'Day Trading', 'Swing Trading', 'HODL']
                    print('1: Scalping', '2: Day Trading', '3: Swing Trading', '4: HODL')
                    nueva_met_op = input('Ingrese el número de la nueva metodología de operación asignada: ')
...
def mostrar_matriz(matriz):
    ...
    print(f"{'NOMBRE':<25} {'TICKER':<8} {'VALOR USD':>12} {'VOLUMEN':>22} {'METODOLOGÍA':>15} {'UNIDADES':>14} {'PUNTAJE':>8}")
    
    print("=" * 110)

    for activo in matriz:
        print(f"{activo[0]:<25} {activo[1]:<8} {float(activo[2]):>12.2f} {float(activo[3]):>22,.2f} {activo[4]:>15} {float(activo[5]):>14,.2f} {int(activo[6]):>8}")
...
def validar_ticker(ticker):
    ...
    if ticker == '' or not ticker.replace('.', '').isalpha() or len(ticker) < 3 or len(ticker) > 5 or ticker.upper() == 'FIN':
...
def validar_volumen(volumen):
    ...
    if volumen == '' or not volumen.isnumeric() or float(volumen) < 0 :
...
def menu():
    ...
    print('='*50)
    print(' '*2 + 'SISTEMA DE GESTIÓN: CRYPTO-ANALYTICS LAB')
    print('='*50)
    ...
    #while activo_a_cambiar.upper() != 'FIN':
        ...
        #for i in lista:
                ...
                #while opcion_modificar != 8:
                        ...
                        #nuevo_nombre = input("Ingrese el nuevo nombre del activo: ")
                        #if nuevo_nombre == '':
                            #print('Error: El nombre no puede estar vacío. No se realizaron modificaciones')
                        #else:
                            #i[0] = nuevo_nombre
                    #elif opcion_modificar == 2:
                        #nuevo_ticker = input("Ingrese el nuevo ticker del activo: ")
                        #if nuevo_ticker == '' or len(nuevo_ticker) < 3:
                            #print('Error: El ticker no puede estar vacío y debe tener al menos 3 caracteres. No se realizaron modificaciones')
                        #else:
                            #i[1] = nuevo_ticker
                        ...
                        #print('1: Scalping', '2: Day Trading', '3: Swing Trading', '4: HODL')
                        #nueva_met_op = int(input(f'Ingrese el número de la nueva metodología de operación asignada: '))
